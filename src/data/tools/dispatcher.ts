...
export function matchIntent(text: string): ToolCall | null {
  ...
  if (/\b(events?|schedule|calendar|agenda|meetings?)\b/.test(t)) {
    if (/\b(show|list|what|my|check|read|view|upcoming|when)\b/.test(t) || t === "my calendar" || t === "upcoming events") {
  ...
  // 11. Route / Directions Fast-Path — the DEFAULT for "how do I get to X"
  // style phrasing. Shows a live route + map card right in chat instead
  // of handing off to the Maps app. Must come before the Maps block below
  // since phrasing overlaps ("to"). Broad on purpose: this natural
  // phrasing space is large, and anything that slips through here falls
  // to the on-device model, which doesn't reliably know real tool names
  // for things it hasn't seen an example of.
  if (
    /\b(how far|how long (to|does it take)|distance to|eta to|travel time to|how (?:do|can) i get to|(?:show me )?(?:the )?way to|route to|directions? to|show me (?:the way|how to get) to|get me to)\b/i.test(
      t
    )
  ) {
    const destMatch = t.match(
      /(?:how far (?:is|to)|how long (?:to|does it take to)?|distance to|eta to|travel time to|how (?:do|can) i get to|(?:show me )?(?:the )?way to|route to|directions? to|show me (?:the way|how to get) to|get me to)\s+(.+)$/i
    );
  ...
  // 12. Maps App Hand-Off Fast-Path — only for phrasing that explicitly
  // asks for the Maps app itself (not just "how do I get there", which
  // rule 11 already covers with an in-chat answer).
  if (/\b(open maps|google maps|navigate to|take me to)\b/.test(t)) {
    const destMatch = t.match(/(?:navigate to|take me to|to)\s+(.+)$/i);
    const destination = destMatch ? destMatch[1].trim() : "";
  ...
  // 15. General Knowledge / Current-Info Fast-Path — catches factual
  // questions Pico has no dedicated tool for ("what is the current price
  // of gold", "when will X be released", "who is the president of Y").
  // This must stay LAST: every more specific rule above (tasks, calendar,
  // weather, maps, route, etc.) gets first refusal, since it returns
  // before reaching here. Anything left over that still reads as a
  // question gets looked up instead of falling through to the on-device
  // model, which is small enough to sometimes guess wrong or misfire an
  // unrelated tool for things it has no real answer for.
  if (/^(what|who|when|how much|how many|which|where)\b/i.test(t) && t.length > 6) {
    const query = t.replace(/[?.!]+$/g, "").trim();
    if (query) {
      return { name: "answer_question", args: { query } };
    }
  }
