...
export function matchIntent(text: string): ToolCall | null {
  ...
  if (
    /^(hi|hello|hey|hola|sup|good morning|good afternoon|good evening|who are you|how are you|what can you do)[\s!.]*$/i.test(
      t,
    )
  ) {
    if (
    ...
    if (t.includes("who are you") || t.includes("what can you do")) {
      return {
        ...
        args: {},
        directMessage:
          "I am Pico, your private on-device AI assistant! I can manage your tasks, schedule events, check the weather, set timers, control your device, and break down complex goals.",
      };
    }
    return {
      name: "__chat__",
  ...
  if (
    /^(what is today|what day is today|what date is today|today's date|current date|what time is it|what's the time|what is the time)[\s?.]*$/i.test(
      t,
    )
  ) {
    const now = new Date();
  ...
  if (/\b(flash\s?light|torch|lamp)\b/.test(t) || /\b(light)\b/.test(t)) {
    if (
      /\b(turn\s*off|switch\s*off|disable|kill|stop|shut\s*off|douse|off)\b/.test(
        t,
      )
    ) {
  ...
  if (/\b(battery|juice|charge|power level|battery percentage)\b/.test(t)) {
    if (
      /\b(how much|what|check|status|level|remaining|is my phone)\b/.test(t) ||
      t === "battery" ||
      t === "battery status"
    ) {
  ...
  if (/\b(tasks|todo|todos|to-do)\b/.test(t)) {
    if (
      /\b(show|list|what|my|check|read|view|get)\b/.test(t) ||
      t === "my tasks" ||
      t === "tasks"
    ) {
  ...
  if (/\b(events|schedule|calendar|agenda|meetings)\b/.test(t)) {
    if (
      /\b(show|list|what|my|check|read|view|upcoming)\b/.test(t) ||
      t === "my calendar" ||
      t === "upcoming events"
    ) {
  ...
  if (
    /\b(daily briefing|morning briefing|brief me|day briefing|summarize my day|what does my day look like|how is my day)\b/.test(
      t,
    ) ||
    t === "briefing" ||
    t === "good morning" ||
  ...
  if (
    /\b(mark|complete|done|finish)\b/.test(t) &&
    /\b(task|todo|as done|as completed)\b/.test(t)
  ) {
    const cleaned = t
      .replace(
        /\b(mark|complete|done|finish|as done|as completed|task|the|my|please)\b/gi,
        "",
      )
      .trim();
    if (cleaned) {
  ...
  if (
    ...
    const rawGoal = t
      .replace(
        /\b(plan my|break down my|break down|help me plan|help me prepare for|create a plan for|generate subtasks for|goal to|my goal to|goal|project|please)\b/gi,
        "",
      )
      .trim();
    if (rawGoal.length > 2) {
  ...
  if (/\b(timer|alarm|countdown)\b/.test(t)) {
    const durationMatch = t.match(
      /(\d+\s*(?:minutes?|mins?|seconds?|secs?|hours?|hrs?|m\b|s\b|h\b))/i,
    );
    const duration = durationMatch ? durationMatch[1] : "15 minutes";
    const labelMatch = t.match(/for\s+(.+?)(?:\s+in|\s+for|\s+at|$)/i);
    const label =
      labelMatch && !labelMatch[1].match(/^\d+/) ? labelMatch[1].trim() : "";
    return { name: "set_timer", args: { duration, label } };
  }

  // 10. Weather Fast-Path
  if (
    /\b(weather|forecast|temperature|umbrella|rain|hot outside|cold outside)\b/.test(
      t,
    )
  ) {
  ...
  // 11. Route / Distance / ETA Fast-Path — answers directly instead of
  // handing off to the Maps app. Must come before the Maps block below
  // since phrasing overlaps ("to").
  if (
    /\b(how far|how long (to|does it take)|distance to|eta to|travel time to)\b/i.test(
      t,
    )
  ) {
    const destMatch = t.match(
      /(?:how far (?:is|to)|how long (?:to|does it take to)?|distance to|eta to|travel time to)\s+(?:get to\s+)?(.+)$/i,
    );
    const destination = destMatch ? destMatch[1].trim() : "";
    if (destination) {
      return { name: "get_route", args: { destination } };
    }
  }

  // 12. Maps / Directions Fast-Path
  if (
    /\b(open maps|google maps|show me the map|directions?|navigate to|take me to)\b/.test(
      t,
    )
  ) {
    const destMatch = t.match(
      /(?:directions?\s*to|navigate to|take me to|to)\s+(.+)$/i,
    );
    const destination = destMatch ? destMatch[1].trim() : "";
    return { name: "open_in_maps", args: { destination } };
  }

  // 13. Play song / video on YouTube Fast-Path
  if (
    /^\s*play\s+/i.test(t) ||
    /\b(youtube|watch a video of|find a video of)\b/.test(t)
  ) {
    const query = t
      .replace(/^\s*(please\s+)?play\s+/i, "")
      .replace(
        /\b(on youtube|youtube|watch a video of|find a video of)\b/gi,
        "",
      )
  ...
  if (
    ...
    const query = t.replace(/\b(search for|google|look up)\b/gi, "").trim();
    if (query) {
